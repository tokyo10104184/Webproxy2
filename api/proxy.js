...
const handleRequest = async (req, res) => {
    ...
    if (method === 'POST') {
        ...
        if (proxy_target_url) {
            ...
            // ユーザーがアドレスバーから直接入力した場合
            const userInput = req.query.url;
            if (userInput && /^https?:\/\//i.test(userInput)) {
                // http:// または https:// で始まる場合はURLとして扱う
                targetUrl = userInput;
            } else if (userInput) {
                // それ以外はEcosiaでの検索キーワードとして扱う
                targetUrl = `https://www.ecosia.org/search?q=${encodeURIComponent(userInput)}`;
            }
        }
    }
